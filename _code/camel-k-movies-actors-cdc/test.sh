...
kubectl exec deploy/elasticsearch -n "$NAMESPACE" -- curl -s "http://localhost:9200/movies-actors-joined/_search?pretty&size=2" 2>/dev/null || echo "Could not query ES"
echo ""

echo "Integration logs (last 20 lines):"
kubectl logs -l camel.apache.org/integration=movies-actors-cdc -n "$NAMESPACE" --tail=20 2>/dev/null || echo "No integration logs found"
echo ""
